...
public class BbsUploadSVCImpl implements BbsUploadSVC{
  ...
  public Long save(BbsUpload upload,MultipartFile file) {
    ...
    String safeBase  = URLEncoder.encode(baseName, StandardCharsets.UTF_8);
  ...
  @Override
  @Transactional
  public List<UploadResult> saveAll(Long bbsId,String fileType,List<MultipartFile> files) {
    // 1) 현재 최대 sort_order 구하고
    int nextOrder = bbsUploadDAO.getMaxSortOrder(bbsId, fileType) + 1;
    ...
    for (MultipartFile file : files) {
      ...
      // 4) 결과 리스트에 추가
      results.add(new UploadResult(id, u.getFilePath()));
    }
    return results;
  }
  ...
  public int getMaxSortOrder(Long bbsId, String fileType) {
    return bbsUploadDAO.getMaxSortOrder(bbsId,fileType);
  }
  ...
  public void deleteById(Long uploadId) {
    ...
    bbsUploadDAO.decrementSortOrders(bbsId, fileType, order);
